...
function App() {
  ...
  const handlePrint = (type) => {
    ...
    // If ID starts with 'pre2-', it's Pre-2nd Grade. Otherwise default to Grade 2.
    const isPre2 = selectedYearSession.startsWith('pre2-');
    const gradeLabel = isPre2 ? '英検準2級' : '英検2級';
    ...
    // Dynamic sizing for ALL types based on request
      ...
      questionsHtmlContent = parsedQuestions.map((q, idx) => `
        <div class="q-row" style="${idx !== parsedQuestions.length - 1 ? 'border-bottom: 1px dashed #ccc;' : ''} padding: 0.6rem 0; display: flex;">
          ...
          <div class="q-content-col" style="padding-left: 1rem; flex-grow: 1;">
            <div class="q-text" style="font-weight: bold; page-break-inside: avoid; margin-bottom: 0.4rem; font-family: 'Times New Roman', serif;">${q.text}</div>
            <div class="q-options" style="font-size: 14px; font-family: 'Times New Roman', serif;">
              ${q.options.map(opt => `<div style="margin-bottom: 0.15rem;">${opt}</div>`).join('')}
            </div>
          </div>
        </div>
      `).join('');
        ...
        <head>
          ...
          <style>
            ...
            .page-column { 
              ...
              height: 100%;
              overflow: hidden;
              padding-bottom: ${isOriginal ? '12mm' : '5mm'};
            }
            .header-label { 
              font-size: 11px; 
            ...
            p { margin-bottom: 0.5em; text-indent: 1em; margin-top: 0; }
            button {
              padding: 10px 20px; 
              font-size: 16px; 
              cursor: pointer; 
