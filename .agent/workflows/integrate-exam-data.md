...
2.  **Verify Syntax Analysis Format**:
    ...
    *   **CONSTRAINT**: **Do NOT bold verbs inside subordinate clauses (e.g., `that` clauses, relative clauses). Only bold the main sentence verbs.**
    *   **DESIGN (Evidence)**: Use `<div class="syntax-panel syntax-panel-evidence">Evidence: "..."</div>`. **Do NOT use `<blockquote>` or inner titles.**
    *   **DESIGN (Strategy)**: Use `<div class="syntax-strategy"><strong>[Strategy Note]</strong> ...</div>`.
